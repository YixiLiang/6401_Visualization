...
# 7 countries
df = df1.loc[:, ['Country/Region', 'US', 'Brazil', 'United Kingdom_sum', 'China_sum', 'India', 'Italy', 'Germany']]
countryName = df.columns[1:]
...
covid_layout = html.Div([
    html.H3('COVID global confirmed cased'),
...
def calculate(firstNumber, secondNumber, calculatorOperator):
    if firstNumber and secondNumber and calculatorOperator is not None:
        if calculatorOperator == 'Addition':
            ...
            result = f'that the log of first number is {math.log(firstNumber)},and the log of second number is {secondNumber}'
        ...
        return html.H6('The result will only display when all of the input are not None', style={'color': 'red'})
...
normal_Distribution_layout = html.Div([
    ...
    html.Br(),
    html.P('Std'),
    dcc.Slider(id='std', min=1, max=3, step=1, value=2,
               marks={1: '1', 2: '2', 3: '3'}),
...
polynomial_layout = html.Div([
    html.H3('Please enter the polynomial order'),
